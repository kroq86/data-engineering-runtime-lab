...
fn tx_journal_path(journal_dir: &Path, tx_id: u64) -> PathBuf {
    journal_dir.join(format!("tx_{tx_id}.journal"))
}

fn create_tx_journal_file(journal_dir: &Path, tx_id: u64) -> Result<(), String> {
    fs::create_dir_all(journal_dir).map_err(|e| format!("create tx_journal dir failed: {e}"))?;
    File::create(tx_journal_path(journal_dir, tx_id))
        .map_err(|e| format!("create tx journal failed: {e}"))?;
    Ok(())
}

fn append_tx_journal_op(journal_dir: &Path, tx_id: u64, op: &TxStagedOp) -> Result<(), String> {
    let mut file = OpenOptions::new()
        .append(true)
        .create(true)
...
fn remove_tx_journal(journal_dir: &Path, tx_id: u64) -> Result<(), String> {
    let path = tx_journal_path(journal_dir, tx_id);
    if path.exists() {
        fs::remove_file(path).map_err(|e| format!("remove tx journal failed: {e}"))?;
    }
    Ok(())
}

fn load_recovery_journals(journal_dir: &Path) -> Result<HashMap<u64, Vec<TxStagedOp>>, String> {
    let mut out: HashMap<u64, Vec<TxStagedOp>> = HashMap::new();
    if !journal_dir.exists() {
        return Ok(out);
    }
    let entries =
        fs::read_dir(journal_dir).map_err(|e| format!("read tx_journal directory failed: {e}"))?;
...
fn parse_snapshot_line(line: &str) -> Result<Option<Row>, String> {
    let parts: Vec<&str> = line.split(',').collect();
    if parts.len() != 3 {
        return Ok(None);
    }
...
fn parse_wal_line(line: &str) -> Result<WalOp, String> {
    let parts: Vec<&str> = line.split(',').collect();
    if parts.len() != 4 {
